...
pub struct LockedSqliteHandle<'a> {
    ...
    handle_raw: ConnectionHandleRaw,
}
...
impl SqliteConnection {
    ...
    /// return an error during the connection creation.
    ///
    /// Or if necessary, you can call [`.lock_database()`][Self::lock_database]
    ...
    pub async fn lock_database(&mut self) -> Result<LockedSqliteHandle<'_>, Error> {
        let handle_raw = self.worker.handle_raw.clone();
        let guard = self.worker.unlock_db().await?;

        Ok(LockedSqliteHandle { guard, handle_raw })
    }
}
